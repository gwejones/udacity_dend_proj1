...
from create_tables import HOST, USERNAME, PASSWORD

def process_song_file(cur, filepath):
    """
    - Reads JSON song metadata file given by filepath
    - Performs ETL into database given by cur
    """
    
    # open song file
    df = pd.read_json(filepath, typ='dict')
...
def process_log_file(cur, filepath):
    """
    - Reads JSON user event log file given by filepath
    - Performs ETL into database given by cur
    """
...
def process_data(cur, conn, filepath, func):
    """
    - Finds all dataset JSON dataset files in folder given by filepath
    - Dispatches the appropriate function to perform ETL on the file
    """
